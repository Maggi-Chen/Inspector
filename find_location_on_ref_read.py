...
#svfile='/data/scratch/maggic/simulation/denovosimulation/halploid_chr1/test_aligenr/test_ngmlr.canu.read_ctg.sam'
svfile='/data/scratch/maggic/simulation/denovosimulation/halploid_chr1/inspector_flye_polished/read_to_contig.sam'
...
def findcontigpos(cigar,start,pos,end,clipinfo,flag):
	num=''
	if int(flag) in [0,2048]:
		target=pos-clipinfo[0]
		target2=end-clipinfo[0]

	else:
		target=clipinfo[1]-pos+clipinfo[2]
		target2=clipinfo[1]-end+clipinfo[2]
	...
	for c in cigar:
		...
		if c in 'M=X':
			if target<int(num):
				location+=target; returnloc[0]=location; break
			else:
	...
	for c in cigar:
		...
		if c in 'M=X':
			if target2<int(num):
				location2+=target2; returnloc[1]=location2; break
			else:
				target2-=int(num); location2+=int(num); num=''
		if c in 'D':
			location2+=int(num);num='';
		if c in 'I':
			target2-=int(num)
			num=''
		if c in 'SH':
...
f=open(svfile,'r')
g=open('/data/scratch/maggic/simulation/denovosimulation/halploid_chr1/test_aligenr/flye_ctg_minimap2.readpos_test','w')
readinfo=f.readline()
iii=0
small_contig=0
...
while readinfo!='':
	...
	if contiginfo!=[]:
		refpos=findcontigpos(contiginfo[5],contiginfo[4],processedinfo[2],processedinfo[3],contiginfo[6],contiginfo[7])
		if contiginfo[7] in [0,2048]:
			ctgrev=False
		else:
			ctgrev=True
		if reverse == ctgrev:
			g.write(readinfo.split('\t')[0]+'\t+\t'+str(processedinfo[0])+'\t'+str(processedinfo[1])+'\t'+contiginfo[3]+'\t'+str(min(refpos))+'\t'+str(max(refpos))+'\n')
		else:
			g.write(readinfo.split('\t')[0]+'\t-\t'+str(processedinfo[0])+'\t'+str(processedinfo[1])+'\t'+contiginfo[3]+'\t'+str(min(refpos))+'\t'+str(max(refpos))+'\n')
