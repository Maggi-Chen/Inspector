...
def detect_sam_ref(filename,readpath,writepath,min_size,max_size):
	...
	f=open('/data/scratch/maggic/simulation/denovosimulation/halploid_chr1/inspector_canu_test/not_unique_readname','w')
	for c in multimap:
		f.write(c+'\n')
	f.close()
...
'''
readpath='/data/scratch/maggic/simulation/denovosimulation/halploid_chr1/inspector_canu_test/'
writepath=readpath
chromosomes=[]
detect_sam('read_to_contig.sam',readpath,writepath,chromosomes,45,10000000)
'''

outpath='/data/scratch/maggic/simulation/denovosimulation/halploid_chr1/inspector_flye_test/'
f=open(outpath+'not_unique_read','r')
allread=f.read().split('\n')[:-1]
f.close()
f=open(outpath+'read_to_contig.debreak.temp','r')
a=f.read().split('\n')[:-1]
f.close()
f=open(outpath+'read_to_contig.debreak.temp_clean','w')
for c in a:
	if 'TRA' not in c and c.split('\t')[4] not in allread:
		f.write(c+'\n')
f.close()
