...
def mergeinfo_insertion_oneevent(candi,min_support):
	...
	while len(candi)>max(2,min_support-2):
		if int(candi[-1].split('\t')[2]) > 2* int(candi[int(len(candi)/2)].split('\t')[2]) and  int(candi[-1].split('\t')[2]) -int(candi[int(len(candi)/2)].split('\t')[2]) >30:
			candi.remove(candi[-1])
			continue
		if int(candi[int(len(candi)/2)].split('\t')[2]) >  2*int(candi[0].split('\t')[2]) and int(candi[int(len(candi)/2)].split('\t')[2]) -int(candi[0].split('\t')[2]) >30:
		...
		for d in col:
			if c[0]==d.split('\t')[0] and int(c[1])-250<=int(d.split('\t')[1])<=250+int(c[2]) and int(c[5].split('=')[1])<20*int(d.split('\t')[5].split('=')[1]):
				...
				if 0.33<=int(c[3])/float(d.split('\t')[3])<=3:
					if totaln<min(goodexp+int(goodcol/2),goodcol+int(goodexp/2)):
