...
def simple(contigfile, outpath, min_size, min_size_assemblyerror):
    ...
    for contig in contigfile:
        ...
        for c in allcontig:
            ...
            contig_name = re.sub(r"[^\w]", "_", contig_name)
            if halp:
    ...
    f.write(
        "Number of contigs > " + str(min_size) + " bp\t" + str(len(map_contigs)) + "\n"
    )
    f.write(
        "Number of contigs >"
        + str(min_size_assemblyerror)
        + " bp\t"
        + str(len(length_ae))
        + "\n"
    )
    ...
    f.write(
        "Total length of contigs > " + str(min_size) + " bp\t" + str(totallength) + "\n"
    )
    f.write(
    ...
    if len(length) == 0:
        ...
        logf.write(
            "Error: No contigs found. Check if input file is empty or if --min_contig_length is too high.\n"
        )
        f.close()
        logf.close()
        quit()
    ...
    if len(length_ae) == 0:
        logf = open(outpath + "Inspector.log", "a")
        logf.write(
            "Warning: No contigs larger than "
            + str(min_size_assemblyerror)
            + "bp. No structural errors will be reported. Check if --min_contig_length_assemblyerror is too high.\n"
        )
        logf.close()
...
def mapping_info_ctg(outpath, largechrom, smallchrom, contiglength, contiglength_large):
    ...
    os.system(
        "find "
        + outpath
        + "map_depth/ -name 'maplength_large_*' -exec cat {} + > "
        + outpath
        + "map_depth/all_maplength_large"
    )
    os.system(
        "find "
        + outpath
        + "map_depth/ -name 'maplength_*' -exec cat {} + > "
        + outpath
        + "map_depth/all_maplength_total"
    )
    os.system(
        "find "
        + outpath
        + "map_depth/ -name 'readnum_large_*' -exec cat {} + > "
        + outpath
        + "map_depth/all_readnum_large"
    )
    os.system(
        "find "
        + outpath
        + "map_depth/ -name 'readnum_*' -exec cat {} + > "
        + outpath
        + "map_depth/all_readnum_total"
    )
    os.system(
        "find "
        + outpath
        + "map_depth/ -name 'splitread_large_*' -exec cat {} + > "
        + outpath
        + "map_depth/all_splitread_large"
    )
    os.system(
        "find "
        + outpath
        + "map_depth/ -name 'splitread_*' -exec cat {} + > "
        + outpath
        + "map_depth/all_splitread_total"
    )
    ...
    try:
        info = (
            open(outpath + "map_depth/all_readnum_large", "r").read().split("\n")[:-1]
        )
        mapped = sum([int(ccc) for ccc in info])
        mapprate = round(10000 * float(mapped) / (totalread)) / 100.0
        ...
        info = (
            open(outpath + "map_depth/all_splitread_large", "r").read().split("\n")[:-1]
        )
        splitread = sum([int(ccc) for ccc in info])
        splrate = round(10000 * float(splitread) / mapped) / 100.0
        ...
        info = (
            open(outpath + "map_depth/all_maplength_large", "r").read().split("\n")[:-1]
        )
        mappedlen = sum([int(ccc) for ccc in info])
        ...
        logf = open(outpath + "Inspector.log", "a")
        logf.write(
            "Warning: Failed to characterize read alignment in large contigs. \n"
        )
        logf.close()
...
def assembly_info_cluster(outpath, min_size, max_size):
    os.system(
        "find "
        + outpath
        + "ae_merge_workspace/ -name 'del_merged_*' -exec cat {} + > "
        + outpath
        + "ae_merge_workspace/deletion-merged"
    )
    os.system(
        "find "
        + outpath
        + "ae_merge_workspace/ -name 'ins_merged_*' -exec cat {} + > "
        + outpath
        + "ae_merge_workspace/insertion-merged"
    )
    os.system(
        "find "
        + outpath
        + "ae_merge_workspace/ -name 'inv_merged_*' -exec cat {} + > "
        + outpath
        + "ae_merge_workspace/inversion-merged"
    )
    f = open(outpath + "assembly_errors.bed", "w")
    alldel = (
        open(outpath + "ae_merge_workspace/deletion-merged", "r")
        .read()
        .split("\n")[:-1]
    )
    ...
    allins = (
        open(outpath + "ae_merge_workspace/insertion-merged", "r")
        .read()
        .split("\n")[:-1]
    )
    ...
    allinv = (
        open(outpath + "ae_merge_workspace/inversion-merged", "r")
        .read()
        .split("\n")[:-1]
    )
...
def assembly_info_ref(outpath):
    ...
    for c in allsv:
        if "Ins" in c or "Dup" in c:
            c = c.split("\t")
            f.write(
                c[0] + "\t" + c[1] + "\t" + str(int(c[1]) + int(c[2])) + "\tExpansion\n"
            )
        if "Del" in c:
            c = c.split("\t")
            f.write(
    ...
    for c in allinv:
        c = c.split("\t")
        f.write(
            c[0] + "\t" + c[1] + "\t" + str(int(c[1]) + int(c[2])) + "\tInversion\n"
        )
    f.close()
    ...
    for chrom in allchrom:
        ...
        while len(refcovered) > 1:
            if refcovered[0][2] <= refcovered[1][1]:
                ...
                i = 0
                ovlpstart = refcovered[i + 1][1]
                ovlpend = min(refcovered[i][2], refcovered[i + 1][2])
                newblock = []
                ...
                newblock += [
                    [
                        ...
                        ovlpstart,
                        ovlpend,
                        refcovered[i][3] + refcovered[i + 1][3],
                    ]
                ]
...
def get_ref_align_info(path, totallength):
    ...
    for c in allali:
        ...
        if c.flag in [0, 2048]:
            ...
            leftclip = (
                leftclipinfo[1] if leftclipinfo[0] == 5 or leftclipinfo[0] == 4 else 0
            )
            leftclip = (
                leftclipinfo[1] if leftclipinfo[0] == 5 or leftclipinfo[0] == 4 else 0
            )
    ...
    f.write(
        "Genome Coverage /% "
        + str(float(base1 + base2 + base3) / totalrefbase)
        + "\nReference base with Depth=0 (including Ns): "
        + str(base0)
        + ";\t"
        + str(base0 / float(totalrefbase) * 100)
        + "%\n"
    )
    f.write(
        "Reference base with Depth=1 "
        + str(base1)
        + ";\t"
        + str(base1 / float(totalrefbase) * 100)
        + "%\n"
    )
    f.write(
        "Reference base with Depth=2 "
        + str(base2)
        + ";\t"
        + str(base2 / float(totalrefbase) * 100)
        + "%\n"
    )
    f.write(
        "Reference base with Depth>2 "
        + str(base3)
        + ";\t"
        + str(base3 / float(totalrefbase) * 100)
        + "%\n"
    )
    f.write(
        "Assembly contig mapping ratio (length) /%" + str(assembly_maplenratio) + "\n"
    )
...
def get_ref_chroms(outpath):
    ...
    a = int(
        subprocess.check_output(
            "awk '$3==0' " + outpath + "contig_to_ref.depth | wc -l", shell=True
        )
    )
    covered = length - a
...
def check_depth_ref(outpath, ref):
    ...
    cov1 = int(
        subprocess.check_output(
            "awk '$3==1' " + outpath + "contig_to_ref.depth | wc -l", shell=True
        )
    )
    cov2 = int(
        subprocess.check_output(
            "awk '$3==2' " + outpath + "contig_to_ref.depth | wc -l", shell=True
        )
    )
    cov3 = int(
        subprocess.check_output(
    ...
    f.write(
        "Coverage:  " + str(1 - round(10000 * float(cov0) / total) / 10000.0) + "\n"
    )
    f.close()
