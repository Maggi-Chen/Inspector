	...
	for iii  in range(len(segments)-1):
		...
		for c in samedirchr:
			...
			if not if_contig:
				...
				if -1500<overlapmap<window_max and leftread[4]-rightread[3]>=max(50,overlapmap):
				...
				if -1500<overlapmap<window_max and (rightread[4]-leftread[3])>=max(1000,overlapmap):
		...
		window=-1500
		for c in samechr:
			if c[3]>primary[3] and c[4]-primary[4]>window:
				leftread=primary
				rightread=c
			elif c[3]<primary[3] and primary[4]-c[4]>window:
				leftread=c
				rightread=primary
			else:
				continue
...
def detect_sam_ref(filename,readpath,writepath,min_size,max_size):
	...
	while c!='':
		...
		if readname!=lastname:
			...
			if True:#cigarinfo[2][1]>=20000 and  cigarinfo[2][1]>=0.05*(sum(cigarinfo[2])):
				segments+=[readinfo]
		c=f.readline()
	if 1<len(segments):
		segmentd=segmentdeletion(segments,min_size,max_size,True)
...
if __name__ =="__main__":
	readpath='/data/scratch/maggic/simulation/denovosimulation/halploid_chr1_noNread/inspector_wtdbg/'
	writepath=readpath
