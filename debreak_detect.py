...
def segmentdeletion_ref(segments,min_size,max_size,if_contig):
	...
	for iii  in range(len(segments)-1):
		...
		for c in samedirchr:
			...
			window=300
			if if_contig:
				window=min(2000,int(leftinfo[1]/2),int(rightinfo[1]/2))
			if abs(rightread[3]-leftread[4])<=window:
				...
				if min_size<=ins_size<=max_size:
					if not priflag:
						svcallset+=[chrom+'\t'+str(min(rightread[3],leftread[4]))+'\t'+str(ins_size)+'\t'+'I-segment'+'\t'+primary[0]+'\t'+str(int(c[1])+int(primary[1]))+'\t'+str(int((int(c[6])+int(primary[6]))/2))+'\t'+str(leftinfo[0]+leftinfo[1])]
					else:
						svcallset+=[chrom+'\t'+str(min(rightread[3],leftread[4]))+'\t'+str(ins_size)+'\t'+'I-segment'+'\t'+primary[0]+'\t'+str(int(c[1])+int(primary[1]))+'\t'+str(int((int(c[6])+int(primary[6]))/2))+'\t'+str(leftinfo[2])]
			...
			if overlap_window<overlapmap<window_max:
				...
				if min_size<=del_size<=max_size:
					if not priflag:
						svcallset+=[chrom+'\t'+str(leftread[4]-max(0,overlapmap))+'\t'+str(del_size)+'\t'+'D-segment'+'\t'+primary[0]+'\t'+str(c[1])+'\t'+str(int((int(c[6])+int(primary[6]))/2))+'\t'+str(leftinfo[0]+leftinfo[1])]
					else:
						svcallset+=[chrom+'\t'+str(leftread[4]-max(0,overlapmap))+'\t'+str(del_size)+'\t'+'D-segment'+'\t'+primary[0]+'\t'+str(c[1])+'\t'+str(int((int(c[6])+int(primary[6]))/2))+'\t'+str(leftinfo[2])]
		...
		for c in samechr:
			...
			if overlap_window<overlapmap<window_max and (rightread[4]-leftread[4])>=max(100,overlapmap):
				...
				if min_size<=inv_size<=max_size:
					if int(leftread[1]) % 32 <16:
						svcallset+=[chrom+'\t'+str(leftread[4])+'\t'+str(inv_size)+'\t'+'INV-segment'+'\t'+primary[0]+'\t'+str(c[1])+'\t'+str(int((int(c[6])+int(primary[6]))/2))+'\t'+str(leftinfo[0]+leftinfo[1])]
					else:
						svcallset+=[chrom+'\t'+str(leftread[4])+'\t'+str(inv_size)+'\t'+'INV-segment'+'\t'+primary[0]+'\t'+str(c[1])+'\t'+str(int((int(c[6])+int(primary[6]))/2))+'\t'+str(leftinfo[2])]
					continue
			...
			if overlap_window<overlapmap<window_max and (rightread[3]-leftread[3])>=max(100,overlapmap):
				...
				if min_size<=inv_size<=max_size:
					if int(leftread[1]) % 32 <16:
						svcallset+=[chrom+'\t'+str(leftread[3])+'\t'+str(inv_size)+'\t'+'INV-segment'+'\t'+primary[0]+'\t'+str(c[1])+'\t'+str(int((int(c[6])+int(primary[6]))/2))+'\t'+str(leftinfo[0])]
					else:
						svcallset+=[chrom+'\t'+str(leftread[3])+'\t'+str(inv_size)+'\t'+'INV-segment'+'\t'+primary[0]+'\t'+str(c[1])+'\t'+str(int((int(c[6])+int(primary[6]))/2))+'\t'+str(leftinfo[1]+leftinfo[2])]
					continue
	return svcallset
	...
	for iii  in range(len(segments)-1):
		...
		for c in samedirchr:
			...
			if abs(rightread[3]-leftread[4])<=window:
				...
				ins_size=rightinfo[0]-leftinfo[1]-leftinfo[0]-overlap
				if min_size<=ins_size<=max_size:
					svcallset+=[chrom+'\t'+str(min(rightread[3],leftread[4]))+'\t'+str(ins_size)+'\t'+'I-segment'+'\t'+primary[0]+'\t'+str(int(c[1])+int(primary[1]))+'\t'+str(int((int(c[6])+int(primary[6]))/2))]
			...
			if overlap_window<overlapmap<window_max:
				del_size=rightread[3]-leftread[4]+overlapmap
				if min_size<=del_size<=max_size:
					svcallset+=[chrom+'\t'+str(leftread[4]-max(0,overlapmap))+'\t'+str(del_size)+'\t'+'D-segment'+'\t'+primary[0]+'\t'+str(c[1])+'\t'+str(int((int(c[6])+int(primary[6]))/2))]
			'''
			#duplication:
			if not if_contig:
				overlapmap=leftinfo[0]+leftinfo[1]-rightinfo[0]
		...
		for c in samechr:
			...
			if overlap_window<overlapmap<window_max and (rightread[4]-leftread[4])>=max(100,overlapmap):
				inv_size=rightread[4]-leftread[4]-overlapmap
				if min_size<=inv_size<=max_size:
					svcallset+=[chrom+'\t'+str(leftread[4])+'\t'+str(inv_size)+'\t'+'INV-segment'+'\t'+primary[0]+'\t'+str(c[1])+'\t'+str(int((int(c[6])+int(primary[6]))/2))]
					continue
			overlapmap=rightinfo[1]+rightinfo[2]-leftinfo[0]
			if overlap_window<overlapmap<window_max and (rightread[3]-leftread[3])>=max(100,overlapmap):
				inv_size=rightread[3]-leftread[3]-overlapmap
				if min_size<=inv_size<=max_size:
					svcallset+=[chrom+'\t'+str(leftread[3])+'\t'+str(inv_size)+'\t'+'INV-segment'+'\t'+primary[0]+'\t'+str(c[1])+'\t'+str(int((int(c[6])+int(primary[6]))/2))]
					continue
	return svcallset
